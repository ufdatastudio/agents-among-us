...
def get_averaged_features(csv_paths, top_n=10):
    ...
    # Average the weights across the provided CSVs (Aggregating SGD + SVM)
...
def format_feat(feat):
    """Formats the feature for LaTeX (escapes underscores, italicizes structural features)."""
    f = str(feat).replace('_', '\\_')
    return f"\\textit{{{f}}}" if f in ['Reported', 'Statement\\_Num'] else f"``{f}''"

def generate_overall_table(sgd_agg, svm_agg, top_n=10):
    """Generates a single table for the overall aggregated features (Table 1)."""
    print("\n" + "="*95)
    print(f"{'TABLE 1: OVERALL LINGUISTIC SIGNALS (SGD + SVM AGGREGATED)':^95}")
    print("="*95 + "\n")
    
    imp, crew = get_averaged_features([sgd_agg, svm_agg], top_n)
    if not imp and not crew:
        print("[Error] Could not load overall aggregated CSVs.")
        return
  ...
  \\begin{tabular}{lr|lr}
    \\hline
    \\multicolumn{2}{c|}{\\textbf{Indicators of Deception}} & \\multicolumn{2}{c}{\\textbf{Indicators of Honesty}} \\\\
    \\textbf{Feature / n-gram} & \\textbf{Weight} & \\textbf{Feature / n-gram} & \\textbf{Weight} \\\\
    \\hline\n"""

    for i in range(top_n):
        i_feat = format_feat(imp[i]['Feature']) if i < len(imp) else ""
        i_weight = f"+{imp[i]['Weight']:.2f}" if i < len(imp) else ""
        
        c_feat = format_feat(crew[i]['Feature']) if i < len(crew) else ""
        c_weight = f"{crew[i]['Weight']:.2f}" if i < len(crew) else ""

        latex_str += f"    {i_feat} & {i_weight} & {c_feat} & {c_weight} \\\\\n"

    latex_str += """    \\hline
  \\end{tabular}
  }
  \\caption{Top linguistic n-grams and structural features aggregated across all experimental configurations and linear classifiers. Positive weights correlate strongly with Imposter text generation; negative weights correlate with Honest text generation.}
  \\label{tab:overall_linguistic_features}
\\end{table}"""
...
def lime_explanations(models_dict, dataset_path, text_col, reported_col, stmt_col, label_col, output_file="results/lime_visualizations/combined_classifiers_report.html"):
    ...
    for model_name, model_path in models_dict.items():
        ...
        # FIX: Map the numeric predictions back to 'H' and 'B'
        # based on your training logic: 1 == 'B', 0 == 'H'
...
if __name__ == "__main__":
    ...
    generate_overall_table(sgd_agg, svm_agg, top_n=10)

    # --- 2. Generate LIME Visualizations ---
    models = {
